import BLOCKS from "./blocks.js"; // BLOCKS 가 중복 되기때문에 오류가 나는듯.
import BLOCKS2 from "./BLOCKS2.js"
// DOM
...
const movingItem2 = {   //  다음 블럭의 타입과 좌표들을 저장하고 있는 함수.
    type2 : "squ",
    direction2 : 0,  //윗 화살표를 눌렀을때 좌우로 돌려줄때 사용할것임.
    top2: 0,
    left2 : 0,
}; 
...
const movingItem = {   //  다음 블럭의 타입과 좌표들을 저장하고 있는 함수.
    type : "",
    direction : 0,  //윗 화살표를 눌렀을때 좌우로 돌려줄때 사용할것임.
    top: 0,
    left : 0,
};
...
function prependNewLine2(){
    ...
    for(let j=0; j < NEXT_COLS; j++){
        const matrix2 = document.createElement("li");
        ul2.prepend(matrix2);
    }
    li2.prepend(ul2)
    nextblock.prepend(li2)
}
function renderBlocks2(){
    const { type2, direction2, top2, left2} = tempMovingItem2;
    console.log(BLOCKS2[type2][direction2])
    BLOCKS2[type2][direction2].forEach(block=>{
        const z = block[0];
        const w = block[1];
        const target2 = nextblock.childNodes[w].childNodes[0].childNodes[z];
        target2.classList.add(type2)
    })
}
